...
#docker ps -a | grep orderservice: |  cut -d" " -f1

runningCount=`docker ps -a -q --filter ancestor=orderservice:ci | wc -l`

if [ $runningCount -gt 0 ]; then
   docker stop $(docker ps -a -q --filter ancestor=orderservice:ci --format="{{.ID}}") > /dev/nul
else
...
cp $WORKSPACE/target/order-service-0.0.1.jar $WORKSPACE/service.jar'''
...
docker build -t orderservice:$VERSION --build-arg PORT=8090 --build-arg JAR_FILE=service.jar .
#docker tag orderservice:$VERSION orderservice:$VERSION'''
        sh '''#Containerize Microgateway
cd /opt/softwareag/microgateway
docker build -t ordermg:$VERSION .
'''
      }
    }
    stage('Deployment') {
...
docker run --rm --name orderservicems -d -p 8090:8090 orderservice:$VERSION
'''
          }
        }
        stage('Start MicroGW') {
...
docker run --rm --name ordermg -d -p 9090:9090 --net=host ordermg:$VERSION
'''
          }
        }
      }
...
timeout 60 bash -c \'while [[ "$(curl -s -o /dev/null -w \'\'%{http_code}\'\' localhost:8090/order)" != "200" ]]; do sleep 5; done\' || false
'''
          }
        }
...
timeout 60 bash -c \'while [[ "$(curl -s -o /dev/null -w \'\'%{http_code}\'\' localhost:9090/gateway/Order/1.0/order)" != "200" ]]; do sleep 5; done\' || false'''
          }
        }
      }
    }
...
test=`curl -s http://apiworldbuild:8090/order/O1 | grep O1 | wc -l`
...
test=`curl -s http://apiworldbuild:9090/gateway/Order/1.0/order/O1 | grep foo | wc -l`
...
#First tag
docker tag orderservice:$VERSION apiworldref:5000/orderservice:$VERSION
docker tag ordermg:$VERSION apiworldref:5000/ordermg:$VERSION

#second push 
docker push apiworldref:5000/orderservice:$VERSION
docker push apiworldref:5000/ordermg:$VERSION'''
      }
    }
...
deployActive=`kubectl get deployments.apps | grep order-service-deployment | wc -l`
...
if [ $deployActive -gt 0 ]; then
   ...
   #Do a rolling update
   kubectl set image deployment.v1.apps/order-service-deployment order-service=apiworldref:5000/orderservice:$VERSION
   kubectl set image deployment.v1.apps/order-service-deployment order-service-sidecar=apiworldref:5000/ordermg:$VERSION
   
   #Now wait for deploy
   kubectl rollout status deployment.v1.apps/order-service-deployment
else
   echo "NEW Deployment"
   #Inject the version                                            
